...
`classify.qtl` <-
function( cross, peak, etrait.coord, data.gmap)
{
...
for ( i in 1:length(peak) ){
	...
	for ( y in 1:length(peak[[i]])){
				...
				if (is.na(start) ){
					...
					if ( names(peak[[i]][y]) != paste(etrait.coord$chr[bool]) )
						...
						for ( w in 1:2 ){ 
							...
							if(posg_left == posg_right) ratio <- posg_left/posp_left
							else ratio <- (posg_left-posg_right)/(posp_left-posp_right)

							ic[w] <- ic[w]*ratio+posp_left
						}
		...
		# NO QTL FOUND
			...
			# OPTIONAL INFORMATIVE SCREEN MESSAGE
			# cat("no QTL found on chromosome ",names(peak[[i]][y]),"\n")
...
attributes(res)$features <-  c(attributes(peak)$features,'type')
