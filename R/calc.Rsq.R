...
# 
# Karl W Broman wrote the functions find.flanking pull.geno parts of R/qtl package
# licensed under the GNU General Public License version 2 (June, 1991)
#
...
`calc.Rsq` <-
function( cross, peak, th=0.001, round )
{
	...
	for (i in seq(length(peak))){
		...
		for (y in 1:length(peak[[i]])){
			if (!is.na(peak[[i]][y])){
				...
				for (z in seq(length(as.vector(peak[[i]][[y]]$mname.peak)))){
					...
					marker <- find.flanking(cross,as.numeric(chr),as.numeric(pos))
					geno <- pull.geno(cross)[,paste(marker$close)]
					geno <- data.frame(geno)
					n<-n+1
					attributes(geno)$names <- paste(trait,'.',y,'.',z,sep='')
		...
		if (ncol(data)< 2) {
			...
			c<-0
			for ( y in seq(data$perf)) if( any(grep("NA",data[y,])) ) c<-c(c,y)
			data <- data[-c,]
			marker <- names(data[-1])

			#FUNCTION 1: WRITE LINEAR MODEL
			...
			#FUNCTION 2: COMPUTE THE R SQUARE
			Rsq <- function( a, th){
				all.rsq<-''
				qtl<-''
				p <-''

				CMR <- a$"Sum Sq"[nrow(a)]	#CMT-CMR
				CMT <- sum(a$"Sum Sq")		#CMR+CME
				...
				for (y in seq(nrow(a)-1)){
					if( a$"Pr(>F)"[y] <= th ){
						...
						# cat(	"\ttrait: ",qtl[y+1],"\tCMT: ",CMT,"\tCME: ",CME,"\tCMR: ",CMR,"\tR²=CME/CMT: ",rsq,"\n",sep='')
					}else{

						# INFORMATIVE SCREEN MESSAGE
						# cat("\tnon significant R square for",row.names(a[y,]),"\n")
				...
				invisible(data.frame(qtl=qtl[-1],rsq=all.rsq[-1],pF=(p[-1])))
			}

			rsq<-Rsq(a=a,th=th)

			if ( ! missing(round) ){
				rsq$rsq <- round(as.numeric(as.vector(rsq$rsq)),round)
				rsq$pF <- signif(as.numeric(as.vector(rsq$pF)),round)
			}
